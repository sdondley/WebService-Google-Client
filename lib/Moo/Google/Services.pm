...
has 'discovery' => ( is => 'ro', default => sub { require Moo::Google::Discovery; Moo::Google::Discovery->new(debug=>shift->debug) }, lazy => 1 );
...
sub generate_one {
  ...
  warn "Generating Resources for ".ref($object)." class" if ($self->debug);
  my $base_class = ref($object);  # $object
  my $service_name = $api;
  my $service_class_name = ucfirst $api;
  # warn $service_name;
  # # warn $s->{version}[0];
  my $service_class = Moose::Meta::Class->create( join('::', $base_class, $service_class_name) ); # Calendar
  my $service_description = $self->discovery->getRest({ api => $service_name, version => $version });
  warn $service_name. " resources :".Dumper $service_description->{resources} if ($self->debug);
  ...
  for my $resource_name (@resources) {
    ...
    my $resource_class = Moose::Meta::Class->create( join( '::', $base_class, $service_class_name, $resource_class_name) ); # Calendar:Events
    ...
    my $methods_hash = $service_description->{resources}{$resource_name}{methods}; # return like { 'get' => 'HASH(0x3deeb48)', 'list' => 'HASH(0x3e18698)', ... }
    ...
    for my $method_name (@methods) {
      $resource_class->add_method(
        $method_name => sub {
          my ($self, $params) = @_;
          $object->request( join('::', $base_class, $service_class_name, $resource_class_name, $method_name ), $params )
        }
      );
    }
